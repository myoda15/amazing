...
def _subcell_code(
    ...
    if not odd_r and odd_c:
        ...
        if (x, line_y - 1) in gen.glyph or (x, line_y) in gen.glyph:
            return _GLYPH
        if frozenset(((x, line_y - 1), (x, line_y))) in edges:
            return _PATH
        return wall if _h_closed(gen, x, line_y) else _FLOOR
    y, line_x = (r - 1) // 2, c // 2
    if (line_x - 1, y) in gen.glyph or (line_x, y) in gen.glyph:
        return _GLYPH
    if frozenset(((line_x - 1, y), (line_x, y))) in edges:
        return _PATH
    return wall if _v_closed(gen, line_x, y) else _FLOOR


def _path_edges(cells: list[Coord]) -> set[frozenset[Coord]]:
    """Return the adjacent cell-pairs joined along the solution path.

    Each pair is the wall opening the path crosses between two
    consecutive cells. Colouring these openings (not just the cell
    centres) draws the path as a continuous line instead of a row of
    disconnected blocks.
    """
    return {
        frozenset((cells[i], cells[i + 1]))
...
def render_to_string(
    ...
    cells_path = gen.solution_cells() if show_path else []
    path = set(cells_path)
    edges = _path_edges(cells_path)
    lines: list[str] = []
    for r in range(2 * gen.height + 1):
        cells = [
            _block(_subcell_code(gen, r, c, path, edges, wall))
